...
```bash
ssh user@server_ip "cd /path/to/project && ./deploy.sh"
```
### Скрипт deploy.sh должен быть предварительно создан и настроен на сервере.
После настройки достатчно запустить данный срипт
```
ssh user@server_ip
cd /path/to/project
./deploy.sh
